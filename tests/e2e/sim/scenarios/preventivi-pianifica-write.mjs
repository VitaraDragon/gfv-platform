/**
 * E2E write — pianifica preventivo conto terzi (crea lavoro da preventivo accettato).
 * Idempotente: marker 9.99 ha (scen. 24–26); se già pianificato, salta.
 * @module tests/e2e/sim/scenarios/preventivi-pianifica-write
 */

import { E2E_PREVENTIVO_WRITE_SUPERFICIE } from './preventivi-write.mjs';
import { runPreventiviAccettaWriteAssertions } from './preventivi-accetta-write.mjs';
 ...
 */
export async function runPreventiviPianificaWriteAssertions(page, expect) {
  ...
  if (!isPianificato) {
    const hasPianificaBtn = (await row.getByRole('button', { name: /Pianifica/i }).count()) > 0;
    if (!hasPianificaBtn) {
      await runPreventiviAccettaWriteAssertions(page, expect);
      await clearPreventiviFilters(page);
      row = markerPreventivoRow(page);
    ...
    await row.getByRole('button', { name: /Pianifica/i }).click();

    await page.getByText('Lavoro creato con successo!', { exact: false }).waitFor({
      timeout: 45_000,
    });
      ...
      (superficie) => {
        return Array.from(
          ...
          (tr) =>
            ...
            /Pianificato/i.test(tr.textContent || '')
        );
      },
  ...
  await expect(row.locator('.badge-success').filter({ hasText: /Pianificato/i })).toBeVisible();
  await expect(row.getByText('Lavoro creato', { exact: false })).toBeVisible();
