...
In runtime, `core/services/tony-service.js` concatena in ordine: **`CORE/utente/guida.md`**, **`CORE/tony/guida-tecnica.md`**, **`TONY/utente/guida.md`**, **`TONY/tony/guida-tecnica.md`**, poi **intersezioni**, poi altri moduli in **`GUIDA/`**, infine **legacy** `guida-app/moduli/`. **Riassunti:** `guida_sintesi`, `guida_sintesi_parco_macchine`, `guida_sintesi_vigneto`, `guida_sintesi_frutteto`, `guida_sintesi_magazzino`, `guida_sintesi_manodopera`, `guida_sintesi_conto_terzi`, **`guida_sintesi_tony`** — dedup primo turno vs `guida_app` (vedi `tony-service.js`).
