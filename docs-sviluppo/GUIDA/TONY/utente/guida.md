...
Questa guida è per chi usa **Tony**, l’assistente in **chat** integrato in GFV Platform: ti aiuta a **capire l’app**, a **orientarti** e, quando il tuo abbonamento lo consente, a **compilare schede**, **aprire pagine** e **filtrare elenchi** senza dover ricordare ogni passaggio.

Tony **non sostituisce** il giudizio dell’azienda su prezzi, contratti o sicurezza: propone e automatizza ciò che è previsto dall’applicazione, usando i **dati reali** del tuo account (e, per i manager, molti dati aggregati dell’azienda).

Se cerchi solo una riga su «cos’è quella icona», vedi anche la **guida Core** (sezione introduttiva sull’assistente). Qui entriamo nel **modulo Tony** con lo stesso stile delle altre guide: **percorso**, **indice** con **Impara qui**, **mini-guide** ed **esempi** concreti.
...
## Mini-guida: chi è Tony e cosa può fare per te

### In due parole

Tony è il **capocantiere digitale** dell’app: parla in modo **semplice e diretto** (come un collega), conosce le **sezioni** di GFV (terreni, diario, magazzino, preventivi, lavori, ecc. **secondo i moduli attivi** sul tuo abbonamento) e può:
...
## Mini-guida: come aprire Tony e usare chat e voce

### Dove si trova

Di solito trovi un **pulsante flottante** (icona / bolla) sul bordo dello schermo: è il **widget Tony**. Toccandolo si apre il **pannello chat**.

- Se **non vedi** il pulsante, può dipendere dal **piano** (es. piano gratuito senza accesso al widget), dal **browser** o dal fatto che la pagina non carichi ancora il widget: aggiorna la pagina o verifica **Abbonamento** con l’amministratore.

### Chat

1. Apri il widget.  
2. Scrivi una **frase chiara** (meglio una richiesta alla volta).  
3. Leggi la risposta: spesso Tony conclude con una **domanda** («Vuoi che apra…?») o un **passo successivo**.
...
## Mini-guida: come parlare a Tony (esempi)

### Esempi che funzionano bene (Tony Avanzato attivo)
...
Per azioni che **cambiano dati** o **navigano**, Tony spesso chiede una **conferma** («Vuoi che apra…?»). Rispondi in modo esplicito: **«Sì»**, **«Apri»**, **«Ok vai»** — così il sistema collega la tua risposta alla domanda precedente.

### Esempi «solo guida» (anche senza automazioni)
...
- **Richieste troppo vaghe** in un solo messaggio («Sistema tutto»): meglio una cosa alla volta.  
- Chiedere dati su **liste che non hai aperto** o che sono ancora in **caricamento**: Tony dirà che non vede ancora i dati o che la lista è vuota — **aggiorna** o apri la pagina giusta.

---

## Mini-guida: Tony e i dati in lista

### Lista visibile

Quando sei su una pagina con **tabella** (clienti, preventivi, terreni, prodotti, movimenti, …), l’app può dire a Tony **cosa c’è in elenco** (riassunto + righe).

In quel caso puoi chiedere:

- «**Quanti** … ci sono?»  
- «**Quali** sono in stato bozza?»  
- «Riassumi **questa** lista.»

Tony deve usare **solo** quei dati: se la schermata dice ancora «caricamento…», aspetta un secondo e riprova.

### Dati «di azienda» (manager)

Per alcune domande (scorte basse, movimenti recenti, elenco terreni, …) Tony può attingere a un **riepilogo aziendale** preparato dal sistema **prima** della risposta. **Operai e capisquadra** hanno in genere un **accesso più stretto**: Tony lo segnala se qualcosa non è disponibile per il tuo profilo.

---

## Mini-guida: Tony per operaio e caposquadra

### Cosa aspettarti

Il profilo **campo** è pensato per **sicurezza e semplicità**:

- Tony ti aiuta soprattutto con **lavori assegnati**, **ore**, **comunicazioni di squadra**, **impostazioni del tuo account**.  
- Non può aprirti **tutta** l’amministrazione o il magazzino manager se la tua utenza non lo prevede: in quel caso ti spiega di **chiedere a un manager**.

### Esempio

«Apri la gestione utenti» da profilo operaio → Tony risponde che **dal tuo profilo** non può aprire quella sezione e ti indica cosa puoi usare al posto (workspace, segna ore, …).

---

## Mini-guida: briefing in dashboard
...
## Dove approfondire (documentazione tecnica)

Chi sviluppa o gestisce l’infrastruttura del prodotto ha **documentazione dedicata** (architettura dell’assistente, roadmap, stato dell’implementazione): **non** serve leggerla per usare Tony ogni giorno in azienda.
