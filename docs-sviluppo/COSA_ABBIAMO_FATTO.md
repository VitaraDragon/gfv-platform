...
**Ultimo aggiornamento documentazione: 2026-09-15 — piano tenant coerente lato server (`functions/tenant-plan.js`) + fulfillment Stripe via webhook `checkout.session.completed`.**
...
- **Da fare in produzione:** `firebase deploy --only functions:stripeWebhook,functions:tonyAsk,functions:tonyAskStream,functions:getTonyAudio,functions:tonyTranscribeAudio,functions:tonyExtractDocument,functions:createStripeCheckoutSession,functions:fulfillStripeCheckout,functions:syncStripeSubscription,functions:getMeteoSede,functions:getMeteoSedeAvanzato,functions:getMeteoTerreni`; nel Dashboard Stripe aggiungere l'evento **`checkout.session.completed`** all'endpoint webhook. Per il tenant dell'amico: controllare `tenants/{id}` → `plan`, `piano`, `modules`, `stripeSubscriptionId`, `status`; se `plan` manca o è `free` con abbonamento attivo, dopo il deploy basta un accesso (o `syncStripeSubscription` dalla pagina Abbonamento).
...
- **Conferma scenario (utente: «il Base lo ha acquistato sicuramente»):** tenant registrato (`piano: 'free'`, nessun `plan`) → checkout Base completato su iPhone → il client non rientra → l'unico scrittore è il webhook `customer.subscription.updated`, che con il codice precedente scriveva **`plan: 'free'` + `stripeSubscriptionId` + `status: 'active'`**. Il widget (fallback `stripeSubscriptionId` ⇒ Base) mostra il FAB, `tonyAsk` legge `plan` = free e rifiuta: è il messaggio visto dall'amico. Coperto da `tests/stripe-checkout-fulfillment.test.js` («tenant registrato Free (solo piano) → plan base, mai plan free»).
