...
**Data**: 2026-09-26 (… **TTS ElevenLabs voce `5zD2eYSLIo8c2zkowMfP`** — 2026-09-26; **nav: portami al meteo ≠ previsioni** — 2026-09-21; **nav: dashboard di modulo ≠ home ERP** — 2026-09-21; **nav reminder: destinazione diversa non conferma l’offerta** — 2026-09-21; **CI snellimento save/tabelle/card** — 2026-09-20; **Inviti: chiusura lettura pubblica + getInvitoPubblico** — 2026-09-18; **Preventivi numero lock + accettazione in transazione** — 2026-09-18; **Magazzino giacenza atomica (increment)** — 2026-09-18; **delete lavoro a cascata nel codice** — 2026-09-18; **Tony Occhi: galleria + HEIC→JPEG** — 2026-09-17; **voce iPhone: recorder su tutto iOS + sblocco TTS** — 2026-09-16; **placeholder `GFVStandaloneReady`** — 2026-09-20; **log debug `window.__TONY_DEBUG`** — 2026-09-19; **disegno 1b terreni clienti CT** — 2026-09-06; …)  
...
| **4** | Iniezione universale | ✅ In corso | Attività, Lavori (entry point da ovunque 2026-03-08), Terreno (OPEN_MODAL+fields), **Nuovo Preventivo** (preventivo-form, 2026-03-24); **Magazzino** prodotto/movimento + save locale + creazione client-side (entrata/uscita/prodotto) + cross-page dashboard (3b-C15…C19, 2026-06-02), prezzo entrata catalogo; **intervista lavoro** ack tipo stem-only E2E ✅ (2026-06-03); **intervista lavoro vocale E2E** ✅ seconda macchina (2026-06-14, 0 CF); **crea lavoro entity-dense locale** inject-first + qualifier produzione/verde ✅ (2026-08-06); **Segna ore** workspace mobile (**3b-C21**) + desktop segnatura-ore (**3b-C22**, motore unificato — 2026-07-11); **form-trattamento** concimazioni/trattamenti campo (…); … |
| **5** | Grafici e report | ⏳ Parziale | APRI_PAGINA statistiche; MOSTRA_GRAFICO da fare |
| **6** | Proattività e memoria | ⏳ Parziale | **§15.5–§15.6** ✅ v1 reminder (dashboard + 6 hub) + **`prezziInAttesa`** Magazzino (`2026-07-21b`); **§15.4** memoria storica da fare |
...
| **Stripe billing (Abbonamento)** | `functions/stripe-billing.js`, `functions/stripe-webhooks.js`, `functions/module-trial.js`, `functions/index.js` | ✅ **2026-06-20:** Checkout Base/modulo/bundle. ✅ **2026-06-21 Fase 1:** `cancelStripeAddon`, `reactivateStripeAddon`, `stripeWebhook`; UI `abbonamento-standalone.html` (sezione disattivati riattivabili, sync moduli client); secret `STRIPE_WEBHOOK_SECRET`; **D5:** revoca accesso immediata + riattivazione fino a scadenza — verifica OK. ✅ **2026-06-22:** **prova gratuita moduli 30 giorni** (`startModuleTrial`, `syncModuleTrials`, `moduleTrials` Firestore, `core/utils/module-access-resolver.js`); anche piano Free; 1 modulo in prova contemporaneo. Handoff Fasi 2–4: `docs-sviluppo/in-sviluppo/abbonamento/BILLING_V2_HANDOFF.md`. ✅ **2026-09-15:** webhook **`checkout.session.completed`** → `applyCheckoutSessionToTenant` (fulfillment server-side, non dipende dal ritorno del client — web app iOS installata); sync subscription piano scrive sempre `plan: 'base'`; **`functions/tenant-plan.js`** `resolveTenantPlanId` (`plan` > `piano`, abbonamento Stripe Base attivo ⇒ Base) usato da tonyAsk/tonyAskStream/getTonyAudio/tonyTranscribeAudio/tonyExtractDocument/meteo/checkout. ⚠️ **`stripeWebhook` in produzione era rotto dal 2026-06-23** (`ReferenceError: handleStripeWebhookRequest is not defined`, import rimosso da `index.js` in `58db8e9`): ripristinato + `tests/functions-index-handlers-defined.test.js`. ✅ **Deploy CF eseguito 2026-09-15 14:47 UTC** (12 funzioni; webhook risponde 400 su firma mancante, non più 500); evento `checkout.session.completed` aggiunto all'endpoint Stripe test. Test: `tests/stripe-billing-deactivation.test.js`, `tests/module-trial.test.js`, `tests/stripe-checkout-fulfillment.test.js`, `tests/tony-tenant-plan-gate.test.js` |
| Cloud Function **getTonyAudio** | `functions/index.js` + `functions/tony-tts-provider.js` | ✅ **2026-09-26:** TTS **ElevenLabs** (voce `5zD2eYSLIo8c2zkowMfP`, modello Flash) se secret **`ELEVENLABS_API_KEY`**; senza chiave o `TONY_TTS_PROVIDER=google` → Chirp 3 **`it-IT-Chirp3-HD-Charon`**. **Fix 500:** body ufficiale (`language_code` + `speed` in `voice_settings`); se ElevenLabs fallisce → fallback Google (niente `INTERNAL` muto). Env `TONY_TTS_ELEVEN_VOICE` / `TONY_TTS_VOICE` / `TONY_TTS_SPEAKING_RATE`. Gate piano Free invariato. Contratto client: MP3 base64. **Spot 2026-08-24** ancora Chirp Charon (rifare clip se si adotta la nuova voce in produzione). Test: `tests/tony-tts-provider.test.js` |
...
| **Nav / filter quick reply (Fase 4)** | `functions/tony-nav-quick-reply.js`, `functions/tony-filter-table-quick-reply.js` | ✅ Binario B: APRI_PAGINA, RIASSUNTO, FILTER_TABLE ovvio, SUM_COLUMN aggregati; gate moduli; fallback Gemini se ambiguo. **2026-07-31:** hub `manodopera` + `statistiche manodopera` in `NAV_TARGET_RULES`; client `TONY_PAGE_MAP` impegni. **2026-09-21:** «già sulla dashboard» solo home ERP; da Meteo si torna in home; dalla home «portami al meteo» apre il modulo (non le previsioni). Gap noti: `oliveto` stub, ~3 frasi B da log, operai/squadre non ancora in B. **Estensione:** `HANDOFF_CONTINUITA_PERFORMANCE_NAV.md` §Priorità 1. Test: `tony-nav-quick-reply.test.js`, `tony-filter-table-quick-reply.test.js`, `tony-page-map-impegni.test.js` |
...
| **Scarico magazzino da trattamenti** | `trattamento-scarico-magazzino-service.js` + trattamenti/concimazioni vigneto e frutteto | ✅ **2026-09-18:** checkbox e sync uscite usano `hasModuleAccessFromTenant` (pagati + trial), non solo `tenant.modules`. Tony su quelle pagine prende `getAvailableModules()`. Test: `tests/trattamento-scarico-magazzino-gate.test.js`, `tests/module-access-resolver.test.js`. **Canary emulator 6/6** (`npm run magazzino:trial-scarico-canary`): login manager, trial → checkbox visibile + uscita creata, trial off → checkbox nascosta. |
| **Giacenza magazzino (increment atomico)** | `prodotti-service.aggiornaGiacenzaProdotto`, `firebase-service.incrementDocumentField`, pagina Movimenti | ✅ **2026-09-18:** niente lettura+scrittura su `giacenza`. `FieldValue.increment` nel servizio (Tony, trattamenti) e in `movimenti-standalone.html` (crea/modifica/elimina). `updateProdotto` non riscrive `giacenza`. Test: `tests/services/giacenza-increment.test.js`. **Canary emulator 6/6** (`npm run magazzino:giacenza-canary`): due uscite parallele 100→88, rinomina+uscita 85, increment UI 75. |
| **Preventivi — numero lock + accettazione transazione** | `preventivo-lock-utils.js`, `preventivi-service.allocatePreventivoNumero` / `transizionaStatoPreventivo`, `nuovo-preventivo-standalone.html`, `preventivi-standalone.html`, CF `aggiornaStatoPreventivoPubblico` | ✅ **2026-09-18:** seq su `tenants/{id}.preventivoSeqByYear.{anno}` in transazione (no max+1 locale). Accetta/rifiuta get+check+write in transazione (non `updatePreventivo`). Link email: transazione Admin. Test: `tests/services/preventivi-lock.test.js`. **Canary emulator 7/7** (`npm run preventivi:lock-canary`): allocate parallelo 001/002, create parallelo 003/004, secondo accept fallisce, accetta+rifiuta su due doc. Path email produzione dopo `deploy:functions`. |
| **Inviti — chiusura lettura pubblica** | `firestore.rules` `/inviti`, CF `getInvitoPubblico`, `invito-service-standalone.fetchInvitoByToken`, `registrazione-invito-standalone.html`, `gestisci-utenti-standalone.html` | ✅ **2026-09-18:** niente `allow read: if true`. Registrazione via callable pubblica (token → Admin, campi sanitizzati). Lista manager solo `tenantId ==` corrente. Update accettazione (email match) invariato. **Live:** `deploy:functions` → client su `main` → `deploy:rules`. Test: `invito-pubblico.test.js`. **Canary emulator 10/10** (`npm run inviti:rules-canary`): unauth 403, manager tenant ok, altro tenant/operaio denied, CF sanitize, client unscoped permission-denied. |
...
| **Flag prova tenant** | `core/config/feature-flags.js` + switch dashboard | ✅ **2026-09-04:** `featureFlags.preview` (non `moduliAttivi`). Allowlist Sabbie Gialle; default prova se mai impostato. Novelty: `zonaLavorataDuePunti`. Switch **Prova (develop) / Pubblicata (main)** in dashboard per manager/admin. Test: `tests/feature-flags.test.js` |
| **Zona due punti (geometria)** | `core/js/zona-lavorata-slice.js` | ✅ **2026-09-04:** tagli allineati ai lati del campo (obb), non alla corda I→F. **PWA:** `enabledAlways` — non si spegne con switch Pubblicata. Test: `tests/zona-lavorata-slice.test.js` |
| **Proposta confine da tap** | `core/dev/obsoleto/proposta-confine/` | **abbandonato** 2026-09-05: no-go SAM/macchie (Sabbie Gialle). Niente pulsante in Terreni. Piano: `obsoleto/strategie-superate/PIANO_PROPOSTA_CONFINE_TAP.md` §7. |
| **Disegno confini 1b** | `terreni-draw-helpers.js` + `terreni-maps.js` | ✅ **2026-09-05** Terreni; ✅ **2026-09-06** terreni clienti CT (`terreni-clienti-standalone.html`, stesso modulo). Niente SAM. Test `terreni-draw-helpers.test.js`. |
| Form Mapping | `core/config/tony-form-mapping.js` | ✅ ATTIVITA, LAVORO, TERRENO, **PREVENTIVO** (preventivo-form / system instruction structured), **PRODOTTO / MOVIMENTO** magazzino (`prodotto-form`, `movimento-form`, 2026-04-02); **`zona-form`** segna zona (`ZONA_SEGMENTO_FORM_MAP`; **2026-09-03** due punti sul perimetro, **2026-09-04** dietro flag prova; disegno a mano fallback; Tony non simula tap), **`ora-form`** segna ore (`SEGNA_ORE_FORM_MAP`) + **`field-workspace-ore-form`** inline mobile (2026-04-18); **`tonyInterviewFieldIds`** per domande oltre i required HTML (2026-04-02); **prodotto**: `prodottoCategoriaRichiedeGiorniCarenza` + `prodottoCategoriaRichiedeDosaggio`; HTML required `prodotto-form-required.js` (2026-06-02); **policy `LAVORAZIONI_DEFAULTS_TONY`** per default meccanico/campi macchina/copertura da terreno (2026-04-08) |
...
| **STT registratore (iOS)** | `core/js/tony/voice-recorder-stt.js`, `functions/tony-transcribe-audio.js` (callable **`tonyTranscribeAudio`**), `tony-service.transcribeAudio` | ✅ **2026-09-15 (CF in produzione `europe-west1`)** + **hardening 2026-09-16b:** Web Speech è muta nelle web app iOS da Home (WebKit 225298) e in Safari dipende dalla dettatura di sistema → `chooseSttEngine` sceglie `recorder` su **tutto iPhone/iPad** (Safari e PWA). `getUserMedia` + `MediaRecorder` + VAD → Gemini audio. Fallback: vincoli mic troppo stretti → `{ audio: true }`; `MediaRecorder.start(timeslice)` rifiutato → `start()`; AudioContext sospeso → clip a durata fissa (niente falso no-speech). TTS: sblocco HTML Audio al tap FAB/mic + `playsInline`. Meta `apple-mobile-web-app-capable` su dashboard/login + inject loader. Test: `tony-voice-recorder-stt` (28), `tony-transcribe-audio` (11). **Da confermare su iPhone reale** (prompt mic a ogni avvio PWA: WebKit 215884) |
| Voice / TTS | `core/js/tony/voice.js` + **`core/js/tony/engine.js`** + **`core/js/tony/stream-tts-chunk.js`** + **`functions/index.js` (`getTonyAudio`)** | ✅ **build widget `2026-08-06c`** — STT: collapse doppio «crea lavoro», lexicon potatura/statura (2026-08-06) + lexicon base 2026-08-04. **Modalità continua:** whitelist spegnimento auto-mode; `completeTtsClip` → reopen mic post-TTS; `playMicTurnCue`; log `[Tony Voice Auto]`; no restore auto-mode al reload (`2026-06-20k`). **TTS stream voce:** `spokenTtsTexts` + `reconcileUnspokenVoiceSegments` (no frasi saltate); SSE forzato in voce (`forceStream`). **STT:** `applyItalianVoiceQuestionPunctuation` (`?` su domande); **`normalizeItalianSttTranscript` / `scoreItalianSttLexicon`** (allah→alla, typo GFV, H isolate; preferenza alternative); **fix orari fusi ITN (2026-07-15) + trust ore tonde CI (2026-07-27):** `reconstructSegnaOraItnClockRange` / `isSegnaOraUntrustedClockTime` — «dalle 18:53» → «dalle 7 alle 19»; ore `:00`/`:30` non spurie (test `tony-segna-ora-time-range` 38/38). **Congedo:** `checkFarewellIntent` — saluti ≠ addio; «Ok grazie» → risposta locale. Test: `tony-voice-pipeline-canary`, `tony-stream-tts-chunk`, `tony-voice-transcript-punctuation`, **`tony-italian-stt-normalize`** |
...
| **Briefing proattivo (dashboard + hub)** | `tony-proactive-briefing-policy.js`, `tony-proactive-signals.js`, `tony-proactive-hub-briefing.js`, `dashboard-standalone.html` + home hub | ✅ **§15.5–§15.6 v1** + **`prezziInAttesa`** (2026-07-21b). Hub Magazzino: prodotti da completare, scorte, bolle senza prezzo. **2026-09-05:** CTA parlata «dimmi «apri»» tolta (mic spento al reminder; non apriva la pagina citata). **2026-09-21:** `tonyWantsProactiveOpenPage` non tratta «portami a {altra pagina}» come conferma del reminder (es. Magazzino vs Validazione ore). |
| Profilo campo (operaio / caposquadra) | `core/js/tony/field-role-guard.js`, `functions/index.js` (tonyAsk `tonyFieldProfile`), `core/mobile/field-workspace-standalone.html` + `field-workspace-controller.js`, **`core/js/tony/tony-segna-ora-local-engine.js`**, `Tony.initContextWithModules(..., { utente_corrente })` su pagine manodopera | ✅ Cloud: senza `buildContextAzienda` se ruoli solo campo; prompt `SYSTEM_INSTRUCTION_TONY_FIELD` (**segna ore «ieri/oggi» / turno** esplicitamente in ambito — 2026-05-06; comandi canone **INJECT_FORM_DATA** / **QUICK_SAVE**, vietato INJECT/SUBMIT — 2026-06-04). Client: whitelist `APRI_PAGINA` / blocco `OPEN_MODAL` ERP; ruoli obbligatori nel contesto (o sessionStorage `gfv_tony_utente_ruoli`) così guard e CF riconoscono il profilo — 2026-04-17. **Workspace capo (2026-05-19):** slide **Valida ore** (tutti i lavori squadra, approva/rifiuta); comunicazioni squadra operaio/capo; ore proprie capo → manager (non in lista validazione capo). **Nav campo (2026-07-17, `2026-07-17d`):** `comunicazioni` + `statistiche lavoratore` → slide mobile; lista comunicazioni operaio mostra **tutti** i messaggi (non solo da confermare); reload su openSlide Tony. **Doppio FAB (`2026-07-17e`):** iframe embed (`lavori-caposquadra?embed=mobile`) non monta Tony — resta solo il FAB del parent field-workspace — ✅ verificato utente 2026-07-17. **Nav slide (`2026-07-17f`):** intercept locale anche ore/lavoro/valida ore; `lavoro campo`; blocco segnatura apre slide Ore (no falso «sei già su segna ore») — ✅ verificato utente 2026-07-17. **Segna ore inline (2026-06-04):** intervista locale 0 CF (orari elastici, pausa, «ok»/«sì»/«salva»); submit reale `#quick-hours-form` → Firestore → validazione manager (E2E **3b-C21** ✅). **Segna ore desktop (2026-07-11):** stesso motore locale su `#ora-form` / `segnatura-ore-standalone.html` — one-shot fascia+pausa, messaggio raggruppato campi mancanti, auto-open modal, save reale; E2E **3b-C22** (`T-FLOW-022`) |
...
| Spegnimento | Solo: tap mic, chiusura pannello, inattività 30 s, congedo vocale, **errore mic fatale** (permesso negato / nessun mic) | `AUTO_MODE_OFF_REASONS` — build `2026-06-20o`; `mic-error` build `2026-09-15a` |
| Motore STT | Web Speech ovunque funzioni; **registratore + CF `tonyTranscribeAudio`** su **tutto iPhone/iPad** (Safari e app da Home). Nessun risultato parziale: testo a fine frase (mic arancione = trascrizione) | `chooseSttEngine` / `createRecorderSpeechRecognition` — build `2026-09-16b`, **da confermare su iPhone reale** |
...
| Artefatti STT italiano | «alla»→«Allah», typo magazzino/terreni, H isolate → post-normalize + preferenza alternative | `normalizeItalianSttTranscript` + `scoreItalianSttLexicon` (2026-08-04); test `tony-italian-stt-normalize` |
...
| **manodoperaGiorno** | ✅ **2026-07-27:** snapshot giorno (roster/KPI/impegni + shortlist materializzata su standby) in `functions/tony-manodopera-giorno-context.js`; fetch condizionale (modulo manodopera + domanda/pagina/domain); quick reply `query_manodopera_giorno`; gate moduli. Tony non ricalcola candidati. Test: `tony-manodopera-giorno-context.test.js` (4) |
...
| Comando | Stato |
|---------|-------|
| APRI_PAGINA | ✅ (**2026-06-13** target `manodopera` / «home manodopera» / «dashboard manodopera» → hub `modules/manodopera/views/manodopera-home-standalone.html`; `gestione operai` / `operai` → pagina operai; deep link `gestione-lavori` invariati; **2026-07-24** CF/`tony-routes` `impegni giornalieri` / `impegni giorno`; **2026-07-31** anche `TONY_PAGE_MAP` + gate client → URL reale `impegni-giornalieri-standalone.html`; **2026-08-02** guard: domande «spiegami / come fare» → strip `APRI_PAGINA`/`OPEN_MODAL` CF + `triggerAction` client — test `tony-howto-nav-guard.test.js`) |
...
| INJECT_FORM_DATA | ✅ attivita-form, lavoro-form, **preventivo-form**, **terreno-form** (`injectTerrenoForm`, gerarchia coltura, affitto — 2026-06-08), **prodotto-form**, **movimento-form** (2026-04-02), **form-trattamento** (checkbox opzionali — 2026-04-09), **zona-form** (segna zona, due punti o a mano, modal aperto — 2026-09-03), **ora-form** (segna ore, modal aperto — 2026-04-18) |
...
| SUM_COLUMN | ✅ terreni |
| RIASSUNTO (tonyGlobalBriefing) | ✅ Dashboard: `buildDashboardRiassuntoText` + ops con nomi (`summary*`). Hub: **non** offrire riassunto nel reminder; se chiesto → `buildHubProactiveRiassuntoReply` (guida Dashboard, no eco conteggi, no «dimmi apri») — 2026-07-21a / **2026-09-05**. «sì/ok» solo dopo offerta riassunto (tipicamente dashboard). |
...
| **Impegni giornalieri** (manager) | **impegni_giornalieri** | ✅ (`impegni-giornalieri-standalone.html`: KPI + items per operaio/lavoro; merge `setContext('page')` + `table-data-ready` — 2026-07-24) |
...
| Voce | Priorità | Note |
|------|----------|------|
| **Handoff — estensione nav binario B** | Media | Hub `manodopera` ✅ (2026-07-31). Restano ~3 messaggi da log + `oliveto` stub + eventuali sinonimi. Guida: **`HANDOFF_CONTINUITA_PERFORMANCE_NAV.md`** §Priorità 1. `npm run tony:perf-review` |
...
| Tony assente in freemium | — | ✅ Loader + shell (2026-06-22d); verifica E2E Free/Base ok. **2026-09-05:** lazy-load ✅ (`TONY_DECISIONI` §3.10). Eventuali pagine che includono ancora `tony-widget-standalone.js` senza shell = debito minore |
| Maps liste (non lavori) | Bassa | Ancora eager su terreni, vendemmia, trattamenti/concimazioni/potatura/raccolta, lavori caposquadra, segnalazione guasti. Pattern da copiare: `ensureGoogleMapsLoaded` Gestione lavori |
| segnala-guasto-form, macchina-form INJECT | Bassa | Subagent Meccanico |
| MOSTRA_GRAFICO | Bassa | |
| Proattività §15.5 (fasce+delta) | — | ✅ **2026-07-20** — `tony-proactive-briefing-policy.js` + `checkGlobalStatus` |
...
**CI snellimento (2026-09-20):** save lavoro Tony di nuovo con conferma + bubble «Lavoro salvato!». Liste colture: `getBasePath` da `gfv-path.js`. Gestione lavori: handler categoria/tipo subito; lista+roster+dropdown dal seed; catalogo in background; `demo-map-privacy.js` in commit. CI `bb15bb7` 7/7 job, Tony mock 17/17, sim:e2e 71/71 (`gestione-lavori-write` 2.4s primo tentativo). Strumentazione debug tolta.
