...
| **STT registratore (iOS PWA)** | `core/js/tony/voice-recorder-stt.js`, `functions/tony-transcribe-audio.js` (callable **`tonyTranscribeAudio`**), `tony-service.transcribeAudio` | ✅ **2026-09-15 (build `2026-09-15a`; CF `tonyTranscribeAudio` in produzione `europe-west1`):** Web Speech è muta nelle web app iOS da Home (WebKit 225298) → `chooseSttEngine(window)` sceglie `recorder` su iOS + standalone: `getUserMedia` + `MediaRecorder` + VAD (RMS, silenzio 900 ms) → Gemini audio → `onresult` finale → `onspeechend` → `onend`. Stessa interfaccia `SpeechRecognition`, nessuna eccezione per pagina. Mic **arancione** = trascrizione in corso. Errori fatali (`not-allowed`, `audio-capture`) spengono l'auto-mode (`mic-error`) — fix anche per Web Speech. Override test `sessionStorage.tony_stt_engine`. Test: `tony-voice-recorder-stt` (23), `tony-transcribe-audio` (11). **Da verificare su iPhone reale** (prompt mic a ogni avvio: WebKit 215884) |
...
| Motore STT | Web Speech ovunque funzioni; **registratore + CF `tonyTranscribeAudio`** su iPhone/iPad web app da Home (Web Speech muta lì). Nessun risultato parziale: testo a fine frase (mic arancione = trascrizione) | `chooseSttEngine` / `createRecorderSpeechRecognition` in `voice-recorder-stt.js` — build `2026-09-15a`, **da verificare su iPhone reale** |
