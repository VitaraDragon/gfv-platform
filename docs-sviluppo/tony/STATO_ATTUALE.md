...
**Data**: 2026-08-26 (… **push ciclo lavoro icona `core/images`** — 2026-08-26; **mappa allarmi manodopera** — 2026-08-02; **roster A2 UI** — 2026-07-31; …)  
...
| **Push ciclo lavoro** (FCM, fuori app) | `core/config/notification-catalog.js`, `notification-policy.js`, `notification-dispatch-core.js`, `functions/notification-dispatch.js`, `core/js/notification-fcm-client.js`, `service-worker.js` | ✅ Catalogo 6 eventi; actor escluso; coda fuori orario. **Live 2026-08-26** Sabbie Gialle: `completato_da_approvare` arriva (titolo/corpo OK). **Icona:** `buildNotificationIconUrl` → `core/images/icon-192x192.png` HTTPS Pages (non `icons/`, cartella vuota). SW risolve contro `registration.scope`. Status bar Android = silhouette bianca (limite OS). Serve **redeploy Functions** per l’URL nel payload FCM. Spec: `docs-sviluppo/da-fare/notifiche/SPEC_NOTIFICHE_PUSH_CICLO_LAVORO.md`. Tony in-app resta `tony-proactive-signals.js`. |
...
| `docs-sviluppo/da-fare/notifiche/SPEC_NOTIFICHE_PUSH_CICLO_LAVORO.md` | Push FCM ciclo lavoro (catalogo, non Tony in-app). Icona: `core/images/icon-192x192.png` URL assoluto |
