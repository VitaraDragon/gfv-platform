...
**Data**: 2026-06-04 (… **Segna ore workspace campo E2E ✅** intervista + save locale 0 CF + validazione manager; alias INJECT/SUBMIT + intercept locale pausa/salva; **Fase 4 performance** nav/filter/RIASSUNTO + cache invalidation ✅ deploy + E2E; **modello Gemini `gemini-2.5-flash`**; **ack tipo stem-only** + **magazzino 3b-C15…C19**)  
...
| **6** | Proattività e memoria | ⏳ Parziale | Dashboard scorte/scadenze/guasti + briefing meteo + chat 8 gg + pianificazione trattamento/**lavorazione terreno** + praticabilità morfologia + **doppia alternativa prima/dopo** (2026-05-22, §19); `condizioniMeteo`; pagina `meteo_dashboard`; UI soglie tenant praticabilità da fare |
...
| Voice / TTS | `core/js/tony/voice.js` | ✅ `expandSpokenUnitsForItalianTTS`; **`prefetchTonyTTS`** warm cache MP3 in parallelo al testo chat (2026-05-23); `main.js` indicatore «Sto controllando…» ~150 ms |
...
| **Validazione ore Manodopera** | `manodopera-ore-validazione-scope.js`, `validazione-ore-standalone.html`, `dashboard-data.js` (`countOreDaValidareManager`), `comunicazioni-squadra-utils.js` (nomi) | ✅ Manager: lavori autonomi + ore caposquadra su squadra; capo: solo operai; nomi capo in tabella Validazione ore (preload caposquadra + fallback `users`) — 2026-05-19 |
