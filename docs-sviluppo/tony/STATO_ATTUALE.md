...
**CI snellimento (2026-09-20):** save lavoro Tony di nuovo con conferma + bubble «Lavoro salvato!» (`tony-form-save-local.js`). Liste colture: import `getBasePath` da `gfv-path.js`. Pagina aziende sim: card da `manifest.json` senza aspettare Ready. Gestione lavori: primo paint = lista + roster; `demo-map-privacy.js` in commit (senza di esso il modulo pagina non partiva in CI). Test: `tests/tony-form-save-local.test.js`; locale `gestione-lavori-write` / `manodopera-admin`.
