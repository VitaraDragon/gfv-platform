...
| **Tony Occhi — acquisizione documenti** | CF + `document-capture.js` + **`document-review-form.js`** + `document-register.js` + `document-product-match.js` + **`document-prodotto-reminder.js`** + `movimenti-service.updateMovimento` + **`document-eur-format.js`** | ⏳ **Fase 0–3** parziale — 📷 → estrazione → **modal revisione** (€, scontrino, fattura, match DDT, **categoria + prodotto auto + reminder**) → Registra. **Piano §17**: step **A–F** ✅; prossimo **G**. Test **40**. Deploy CF obbligatorio. |
