...
**CI snellimento (2026-09-20):** save lavoro Tony di nuovo con conferma + bubble «Lavoro salvato!» (`tony-form-save-local.js`). Liste colture: import `getBasePath` da `gfv-path.js`. Pagina aziende sim: card da `manifest.json` senza aspettare Ready. Gestione lavori: primo paint dopo `getDocs` (repair/macchine dopo); secondo `applyFilters` a roster caricato. Test: `tests/tony-form-save-local.test.js`.
