...
**Data**: 2026-07-24 (… **vista impegni giornalieri** manodopera — 2026-07-24; **Archivio Occhi Magazzino MVP** + **`prezziInAttesa`** §15.6, build `2026-07-21b` — 2026-07-21; **proattività reminder v1** hub UX + affitti/approvare/sospesi — 2026-07-21; …)  
**Fonte**: codice + `TONY_DECISIONI_E_REQUISITI.md` (… **vista impegni giornalieri** — 2026-07-24; **§20.33 archivio** + **`prezziInAttesa`** — 2026-07-21; **hub navigazione manodopera manager** — 2026-06-13; …)  
...
| **Sostituzioni manodopera** (assenze → shortlist → sostituto) | `manodopera-sostituti-shortlist-*.js`, `lavoro-sostituzione-assenza-service.js`, `manodopera-sostituzione-policy-config.js`, `gestione-lavori-sostituti-ui.js`, assenze/standby | ✅ **Oltre MVP (2026-07-22):** shortlist esclude assenti confermati; Libero/Spostabile/Impegnato + policy priorità/prestito; doppio movimento tracciato; equipaggio minimo (banner); sostituzione anche su lavoro di squadra (`equipaggioGiorno`, no edit squadra globale). **Da fare:** roster completo; **Tony/Context Builder** legge shortlist materializzata (non ricalcola). Piano: `docs-sviluppo/da-fare/tony/PIANO_SOSTITUZIONE_MANODOPERA_SQUADRE.md` |
| **Vista impegni giornalieri** (manager) | `manodopera-impegni-giorno-logic.js`, `manodopera-impegni-giorno-service.js`, `impegni-giornalieri-standalone.html`, hub + Gestione lavori + `tony-routes.json` | ✅ **2026-07-24:** foto del giorno Libero/Impegnato/Assente/Prestato/Sostituto + aggregato per lavoro; solo lettura. **Ingresso:** hub → card Impegni giornalieri; Gestione lavori → Impegni giorno; Tony «apri impegni giornalieri». `currentTableData` `impegni_giornalieri`. Test: `manodopera-impegni-giorno.test.js` |
...
| Comando | Stato |
|---------|-------|
| APRI_PAGINA | ✅ (**2026-06-13** target `manodopera` / «home manodopera» / «dashboard manodopera» → hub `modules/manodopera/views/manodopera-home-standalone.html`; `gestione operai` / `operai` → pagina operai; deep link `gestione-lavori` invariati; **2026-07-24** `impegni giornalieri` / `impegni giorno` → `impegni-giornalieri-standalone.html`) |
