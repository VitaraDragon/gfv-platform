...
**CI snellimento (2026-09-20):** save lavoro Tony di nuovo con conferma + bubble «Lavoro salvato!» (`tony-form-save-local.js`). Liste colture: import `getBasePath` da `gfv-path.js`. Pagina aziende sim: card da `manifest.json` senza aspettare Ready. Gestione lavori: primo paint = solo lista + roster (`currentTableData` dopo `getDocs`); catalogo/macchine/stats dopo. Test: `tests/tony-form-save-local.test.js`; locale `gestione-lavori-write` / `manodopera-admin`.
