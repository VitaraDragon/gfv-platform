...
**Data**: 2026-06-19 (… **Tony consigliere moduli** Base + gating legacy; **TTS latenza** build `2026-06-19a`; **handoff marketing**; briefing dashboard PWA; fix loop mic `2026-06-14b`; …)  
...
| **6** | Proattività e memoria | ⏳ Parziale | … **Tony consigliere moduli** (Base, segnali + complementi — 2026-06-19); **TTS** chunking frasi + pipeline latenza (`2026-06-19a`); … |
...
| **TTS canary (CLI + browser)** | `scripts/tony-tts-canary.mjs`, `npm run tony:tts-canary`, `window.__tonyTtsCanary()` | ✅ Verifica build **`2026-06-19a`**, wiring voice/main/chunk, `speakingRate` 1.05, bundle locale. Test: `tests/tony-tts-latency-canary.test.js` (5) |
...
| Voice / TTS | `core/js/tony/voice.js` + **`functions/index.js` (`getTonyAudio`)** + **`core/js/tony/stream-tts-chunk.js`** | ✅ **Chirp 3 HD Charon**; **`speakingRate` default 1.05** (2026-06-19); **build client `2026-06-19a`**; cache MP3 testo+voice; **callable cached** + **payload minimale** + **dedup in-flight** + **warm init/typing**; **`speakTextInSentenceChunks`**; **`__tonyTtsCanary()`**; prefetch; generation token + barge-in; test `tony-stream-tts-chunk.test.js`, `tony-voice-pipeline-canary.test.js`, `tony-tts-latency-canary.test.js` |
