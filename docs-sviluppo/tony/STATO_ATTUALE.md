...
| **Sostituzioni manodopera** (assenze → shortlist → sostituto) | `manodopera-sostituti-shortlist-*.js`, `lavoro-sostituzione-assenza-service.js`, `manodopera-sostituzione-policy-config.js`, `gestione-lavori-sostituti-ui.js`, assenze/standby | ✅ **Oltre MVP (2026-07-22):** shortlist esclude assenti; Libero/Spostabile/Impegnato; prestito; equipaggio minimo; `equipaggioGiorno`. **2026-07-25 test locale:** lab 5 aziende (`sim:run:sostituzioni-lab`) + canary Playwright **14/14** (`manodopera:sostituzione-canary` = assenza→sostituto→ore→chiusura→approva). Doc: `COSA_ABBIAMO_FATTO` + `simulator/PLAYBOOK_SOSTITUZIONI_LAB.md`. **Da fare:** roster completo; Tony/Context Builder su shortlist; gate CI GitHub dedicato (oggi solo locale). Piano: `docs-sviluppo/da-fare/tony/PIANO_SOSTITUZIONE_MANODOPERA_SQUADRE.md` |
