...
**Data**: 2026-06-20 (… **fix TTS doppio mobile/PWA** build `2026-06-20a`; **Tony consigliere moduli** Base + gating legacy; **TTS latenza**; …)  
...
| **6** | Proattività e memoria | ⏳ Parziale | … **Tony consigliere moduli** (Base, segnali + complementi — 2026-06-19); **TTS** chunking frasi + pipeline latenza; **fix doppio TTS stream mobile/PWA** (`2026-06-20a`); … |
...
| **Consigliere moduli (Tony Guida, Base)** | `functions/tony-module-recommendations.js`, `functions/config/tony-module-recommendations.json`, mirror `core/config/tony-module-recommendations.*` | ✅ **2026-06-19:** segnali terreni/clienti/macchine → `azienda.consigliModuli`; complementi se modulo già attivo; quick reply «quali moduli…»; regole prompt non invasive; **solo piano Base** (non Free); **gating legacy** (no falsi positivi Conto Terzi disattivo) + hint **`reactivate`**. Test: `tests/tony-module-recommendations.test.js` (9) |
| **TTS canary (CLI + browser)** | `scripts/tony-tts-canary.mjs`, `npm run tony:tts-canary`, `window.__tonyTtsCanary()` | ✅ Verifica build **`2026-06-20a`**, wiring voice/main/chunk, `speakingRate` 1.05, bundle locale. Test: `tests/tony-tts-latency-canary.test.js` (5) |
...
| Voice / TTS | `core/js/tony/voice.js` + **`functions/index.js` (`getTonyAudio`)** + **`core/js/tony/stream-tts-chunk.js`** | ✅ **Chirp 3 HD Charon**; **`speakingRate` default 1.05** (2026-06-19); **build client `2026-06-20a`**; **fix doppio TTS** stream→onComplete (remainder su `lastCleanText`, no reset consumed — 2026-06-20); cache MP3 testo+voice; **callable cached** + **payload minimale** + **dedup in-flight** + **warm init/typing**; **`speakTextInSentenceChunks`**; **`__tonyTtsCanary()`**; prefetch; generation token + barge-in; test `tony-stream-tts-chunk.test.js` (9), `tony-voice-pipeline-canary.test.js`, `tony-tts-latency-canary.test.js` |
...
| **consigliModuli**, **segnaliAziendaModuli** | ✅ Suggerimenti moduli da attivare/complementi (`tony-module-recommendations.js`); solo piano Base in `tonyAsk` (2026-06-19) |
