...
| **Tony Occhi — acquisizione documenti** | CF + review + register + sicurezza A+B + prezzo medio + **archivio** + reminder prezzi | ✅ **Fase 0–3 + Archivio MVP + `prezziInAttesa`** (2026-07-21b). 📷 → revisione → Registra → Storage/`documentiAcquisiti` + lista Magazzino (Acquisito + Data doc, Elimina, Apri/Stampa) + link Movimenti. Reminder bolle scoperte su dashboard/hub. Futuro §20.32; P7 retention. **Brand «Occhi» solo interno** (dev/doc tecnica); UI e guide utente: foto/PDF + Tony in prima persona (2026-08-02). |
...
**CI snellimento (2026-09-20):** save lavoro Tony di nuovo con conferma + bubble «Lavoro salvato!» (`tony-form-save-local.js`). Liste colture: import `getBasePath` da `gfv-path.js`. Pagina aziende sim: card da `manifest.json` senza aspettare Ready. Gestione lavori: lista + roster subito (catalogo in parallelo); `currentTableData` e tabella prima dell’import VM; repair/macchine in background. Test: `tests/tony-form-save-local.test.js`; locale T-PERF-002 e `gestione-lavori-write` / `manodopera-admin`.
