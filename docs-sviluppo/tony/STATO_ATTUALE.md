...
**Data**: 2026-09-01 (inventario cerchi aperti sul codice — `docs-sviluppo/CERCHI_APERTI_2026-09-01.md`; … **push S5 assenze + WhatsApp** — 2026-08-28; **push ciclo lavoro icona `core/images`** — 2026-08-26; **mappa allarmi manodopera** — 2026-08-02; **roster A2 UI** — 2026-07-31; …)  
**Fonte**: codice + `TONY_DECISIONI_E_REQUISITI.md` (… **§11.6 allarmi/mappa** — 2026-08-02; **manodoperaGiorno CB** — 2026-07-27; …)  
...
| **Tony Occhi — acquisizione documenti** | CF + review + register + sicurezza A+B + prezzo medio + **archivio** + reminder prezzi | ✅ **Fase 0–3 + Archivio MVP + `prezziInAttesa`** (2026-07-21b). 📷 → revisione → Registra → Storage/`documentiAcquisiti` + lista Magazzino (Acquisito + Data doc, Elimina, Apri/Stampa) + link Movimenti. Reminder bolle scoperte su dashboard/hub. **Must-have (verifica 2026-09-01):** M2 totali + M4 confidence + M6 link ✅; **M1 duplicati / M3 unità / M5 audit estratto vs confermato / P7 retention** aperti. Giacenza alla bolla implicita (entrata aggiorna stock). Futuro §20.32. |
...
| INJECT_FORM_DATA | ✅ attivita-form, lavoro-form, **preventivo-form**, **terreno-form** (`injectTerrenoForm`, gerarchia coltura, affitto — 2026-06-08), **prodotto-form**, **movimento-form** (2026-04-02), **form-trattamento** (checkbox opzionali — 2026-04-09), **zona-form** (traccia segmento, modal aperto — 2026-04-18), **ora-form** (segna ore, modal aperto — 2026-04-18). **Non in mapping:** `segnala-guasto-form`, `macchina-form`. Il prompt CF cita `SAVE_FAULT` — **comando assente nel client** (verifica 2026-09-01) |
...
| MOSTRA_GRAFICO | ❌ Da fare — **zero occorrenze nel repo** (verifica 2026-09-01) |
...
| **Documenti acquisiti** (Magazzino) | **documenti_acquisiti** | ✅ (`documenti-acquisiti-standalone.html` — 2026-07-21) |
| **Calcoli salvati VM** | **calcoli-vendemmia-meccanica** | ✅ (`calcoli-salvati-standalone.html` + `publishTonyTableData` — verifica 2026-09-01) |

**Liste ancora senza `currentTableData` (2026-09-01):** vigneti, frutteti, potatura, trattamenti, raccolta, validazione ore, squadre, operai, compensi, statistiche, tariffe VM, bilancio VM, report.
...
| Voce | Priorità | Note |
|------|----------|------|
| **Handoff — estensione nav binario B** | Media | Hub `manodopera` ✅. Restano oliveto/VM/report/tracciabilità + ~3 messaggi da log. Inventario: **`docs-sviluppo/CERCHI_APERTI_2026-09-01.md`**. Guida: **`HANDOFF_CONTINUITA_PERFORMANCE_NAV.md`** §Priorità 1. `npm run tony:perf-review` |
| **Handoff — metriche client `tony_local_intercept`** | Media | Solo flag E2E in `main.js`; nessun contatore produzione. Guida: **`HANDOFF_CONTINUITA_PERFORMANCE_NAV.md`** §Priorità 2 |
| **Parser terreno CF** | — | **Cablato** in `functions/index.js` (`tryTonyTerrenoEntityParse`, 2026-06-14). Deploy produzione da verificare, non è un gap di codice. |
...
| segnala-guasto-form, macchina-form INJECT | Bassa | Form HTML esistono; **non** in `tony-form-mapping.js`. Prompt CF chiede `SAVE_FAULT` inesistente sul client |
| MOSTRA_GRAFICO | Bassa | Comando assente in tutto il repo (2026-09-01) |
