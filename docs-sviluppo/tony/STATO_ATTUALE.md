...
| **Tony Occhi — acquisizione documenti** | CF + `document-capture.js` + **`document-review-form.js`** + `document-register.js` + `document-product-match.js` + **`document-prodotto-reminder.js`** + `movimenti-service.updateMovimento` + **`document-eur-format.js`** + prezzo medio anagrafica | ⏳ **Fase 0–3** parziale — 📷 → revisione → Registra. **2026-07-19 ✅:** prezzo medio ponderato in anagrafica (auto al load prodotti + fattura/scontrino; pulsante ripiego); fix dropdown «Movimento bolla». **Piano §17**: A–F ✅; prossimo **G**. Hard refresh JS (Tony build `2026-07-19a`). |
