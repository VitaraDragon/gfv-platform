...
**Data**: 2026-05-15  
**Fonte**: codice + `TONY_DECISIONI_E_REQUISITI.md` (ultima verifica incrociata codice-doc guida: 2026-05-11 per §10 **`TONY/`** in `GUIDA_LOAD_ENTRIES`, **`guida_sintesi_tony`**, regola guida **9** e **`SUBAGENT_TONY_MODULO`**; stessa data per `CONTO_TERZI` / `guida_sintesi_conto_terzi`; briefing dashboard vocalmente solo con modulo `tony` — 2026-05-10)  
...
| **6** | Proattività e memoria | ⏳ Parziale | Dashboard + Guasti ok; magazzino liste/form con proattività parziale (timer, interview, conferma salvataggio); **Segna ore** workspace mobile (`#quick-hours-form`): timer post-inject + contesto form con lavoro/data/orari per CF; parsing fascia **due turni** (inizio + «alle 18»), sanitize testo «ore segnate» se form ancora vuoto, **interview pausa** se 0 e chat non ha menzionato pausa (2026-04-18); "Ho notato X" da fare |
...
| **Workspace mobile campo** | **field_workspace** | ✅ (liste + «quali lavori» locale; **segna ore**: **`ora-lavoro`** da nome nel messaggio; select con **reload+finestra** se id Tony assente dal dropdown operaio; blob parsing **ultimi 6 turni**; domanda pausa proattiva in **minuti** — 2026-05-06) |
