...
**Data**: 2026-06-15 (… **briefing dashboard PWA** + **RIASSUNTO nomi ops**; **fix loop mic auto-mode** build `2026-06-14b`; **merge multi-PC** + E2E vocale lavoro; **latenza baseline** `2026-06-14a` …)  
...
| Voice / TTS | `core/js/tony/voice.js` + **`functions/index.js` (`getTonyAudio`)** | ✅ **Chirp 3 HD Charon (2026-06-13)**; cache MP3 keyed su **testo + voice**; `expandSpokenUnitsForItalianTTS`; **`normalizeTemperaturesForItalianTTS`**; **`prefetchTonyTTS`**; **`clearTonyAudioPipeline`** + **`__tonyGeneration`**; **mic off durante TTS** + no barge-in eco (2026-06-09e); **`stream-tts-chunk.js`**; **latenza auto-mode ↓** build `2026-06-14a`; **riapertura mic unificata** `scheduleMicReopenInAutoMode` (`2026-06-14b`); test `tony-stream-tts-chunk.test.js`, `tony-voice-pipeline-canary.test.js` |
