...
**Data**: 2026-06-15 (… **briefing dashboard PWA** — apertura chat mobile + `chatHistory` per «sì»; **RIASSUNTO client con nomi** prodotti/guasti/scadenze via `dashboard-tony-briefing-text.js`; **merge multi-PC** ✅ Chirp 3 + intervista lavoro + hub manodopera + lavoro locale; **E2E vocale lavoro** ✅ seconda macchina `build 2026-06-14a`; **Terreno** entity parser client + CF early exit; **tony-service** HTTP callable + SSE abort; **Latenza auto-mode** baseline 220/450/350/100 ms …)  
...
| Cloud Function **getTonyAudio** | `functions/index.js` | ✅ Google Cloud TTS MP3 base64; voce **`it-IT-Chirp3-HD-Charon`** (Chirp 3 HD — 2026-06-13); env **`TONY_TTS_VOICE`** / **`TONY_TTS_SPEAKING_RATE`**; rollback `it-IT-Wavenet-D`; gate piano Free |
...
| **Briefing meteo dashboard** | `core/js/dashboard-meteo-briefing.js`, `dashboard-standalone.html` (`checkGlobalStatus`), **`dashboard-tony-briefing-text.js`** | ✅ Consigli pioggia + **`weatherSummary`** (oggi/domani sede) in `tonyGlobalBriefing`; saluto proattivo anche se `total===0` criticità ops; voce solo Tony Avanzato + manager/admin — 2026-06-09g; **mobile/PWA (2026-06-15):** apertura pannello chat + `chatHistory`; **RIASSUNTO (2026-06-15):** nomi prodotti sotto scorta, guasti e scadenze mezzi via summary snapshot |
