...
| 20.2 | **Un solo componente upload**: file picker `image/*` + `application/pdf` + **XML FatturaPA** (`.xml`); opz. `capture="environment"` | prodotto 2026-07-10; esteso 2026-09-11 | **implementato** | MVP; no camera live custom; XML = canale fiscale |
...
| 20.8 | **Vision-first Gemini** su foto/PDF; **XML FatturaPA** quando disponibile (parser deterministico, no LLM). OCR pipeline separata (trascrizione) **sì** per accuratezza numeri | prodotto 2026-07-10; raffinato 2026-09-11 | **implementato** | `GEMINI_DOCUMENT_MODEL` opz.; default Flash |
...
| 20.34 | **Fattura elettronica XML (FatturaPA)** è un canale di prima classe: parser deterministico dello schema Agenzia Entrate (non template per fornitore). Se il file è XML/FatturaPA, **non** si chiama Gemini. Conferma umana sul form resta obbligatoria (§20.5) | prodotto 2026-09-11 | **implementato** | `functions/config/tony-fatturapa.js`; anche XML in chiaro dentro PDF |
...
| 20.36 | **Niente IDP di terze parti in v1** (Rossum, Azure Document Intelligence, Google Document AI Invoice, Mindee, Data Alchemy, …). Valutati: utili su volumi enterprise o DDT carta ancora deboli **dopo** XML+due passate. Non mescolare un secondo schema di uscita | prodotto 2026-09-11 | **pianificato** | Riaperto solo se il paper DDT resta insufficiente in campo |
