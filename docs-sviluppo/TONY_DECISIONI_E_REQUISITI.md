...
| 3.10 | **Lazy-load widget client**: FAB placeholder subito; `tony-widget-standalone.js` (injector + `main.js`) a idle (max 2,5 s) o al tap. Fase 2 invariata (Tony da ogni pagina). E2E eager (`?tonyE2e=1`). Intro.js CDN non più scaricato (tour disabilitato, moduli `*-tour.js` restano in repo) | prodotto 2026-09-05 | **implementato** | `gfv-tony-loader.js` |
...
| 14.6f | **Intervista lavoro client-side:** risposte brevi terreno/operaio/data/durata/tipo → patch DOM senza CF; domanda proattiva locale | tony-form-injector, tony/main.js | implementato | `applyLavoroInterviewFromUserReply`, `promptLavoroInterviewMissing`; durata «un giorno» (2026-06-14); test `tony-lavoro-interview-client.test.js` (**45**) — 2026-05-26 / 2026-06-14 / 2026-08-06 |
...
| 14.6g | **Domanda manuale/meccanica (intervista lavoro):** solo stem in `manualMechChoiceStems` (potatura, vendemmia) **e** catalogo terreno con **entrambe** le varianti; stem in `manualMechSkipStems` (erpicatura, trinciatura, …) salta livello 1; **qualifier** «produzione»/«verde» implica modo (`inferTipoModoFromSubtypeQualifier`) senza chiedere M/M | `LAVORAZIONI_DEFAULTS_TONY`, `lavoroTipoStemNeedsManualMechChoice` | implementato | 2026-05-31 — confermato canary vendemmia squadra; **2026-08-06** subtype qualifier |
| 14.6o | **Crea lavoro entity-dense (flusso locale):** se il primo messaggio ha più campi, inject di tutti quelli chiari **prima** di una sola domanda residua (M/M o disamb.); dopo risposta breve, merge di `__tonyLavoroPendingCreationText` (anche assignee) | tony-form-injector (`injectLavoroInterviewPatchNow`, `mergePendingLavoroCreationIntoPatch`) | implementato | 2026-08-06 — fix regressione intervista a pezzi |
...
| 20.2 | **Un solo componente upload**: file picker `image/*` + `application/pdf`; opz. `capture="environment"`. XML FatturaPA **accettato in silenzio** se arriva, non pubblicizzato in UI | prodotto 2026-07-10; raffinato 2026-09-11 | **implementato** | Ingresso utente = foto/PDF; XML extra |
...
| 20.8 | **Vision-first Gemini sulla foto/PDF** (promessa 📷). Due passate (trascrizione + JSON) per i numeri, dal 2026-09-14 in parallelo + reconcile testo. XML FatturaPA solo se il file è già quello — non sostituisce lo scatto | prodotto 2026-07-10; raffinato 2026-09-11 / 2026-09-14 | **implementato** | `GEMINI_DOCUMENT_MODEL` opz.; default Flash; thinking off |
...
| 20.35 | **Due passate Gemini su foto/PDF**: (1) trascrizione verbatim cifra-per-cifra (2) estrazione JSON con `responseSchema` + immagini. Dal 2026-09-14: in **parallelo**, poi reconcile **solo testo**. Level B invariato se totali incoerenti. Nessun layout per fornitore (§20.15) | prodotto 2026-09-11; latenza 2026-09-14 | **implementato** | CF `tonyExtractDocument`; timeout 180 s |
...
| 20.37 | **Latenza acquisizione senza tagliare le cifre:** `thinkingBudget: 0` su Flash (niente ragionamento nascosto); OCR e JSON vision insieme; allineamento numeri con chiamata testo. Non si torna a una sola passata vision | prodotto 2026-09-14 | **implementato** | `runTwoPassExtraction`; callable client 180 s |
...
|--------|---------|-------------|
| **Form acquisizione (Tony 📷)** | Completa il gestionale | Dati strutturati → movimenti, qty, prezzi, prodotti, collegamento bolla↔fattura, fornitori, ecc. |
| **Archivio Magazzino** | Consultazione / controllo / ristampa | **Originali** (foto o PDF caricati dall’utente), non il form di revisione |
